...
export class CheckPermissionsPolicy extends Policy {
  checkModel(req, res, next) {
    ...
    if (user) {
      this.app.services.PermissionsService.isUserAllowed(user, modelName, action)
        .then(permission => {
      ...
      this.app.services.PermissionsService.isAllowed(defaultRole, modelName, action)
        .then(permission => {
          if (!permission || permission.length === 0) {
            res.forbidden(`You don't have permissions to ${action} ${modelName}`)
          }
  ...
  /**
   * Check the permissions on a route
   */
